...
def fix_clinical_files(sample_file, patient_file):
    ...
    # First check if we need to fix header issues in sample file
    ...
    # Next, check for properly formatted entries to understand the structure
    proper_entries = []
    with open(sample_file, 'r') as f:
        for line in f:
            if not line.startswith('#') and not line.startswith('SAMPLE_ID'):
    ...
    if proper_entries:
        ...
        expected_cols = len(proper_entries[0].strip().split('\t'))
    ...
    for line in patient_lines:
        if not line.startswith('#') and not line.startswith('AGE'):
            ...
            if len(parts) >= 1 and parts[0] and re.match(r'^P[A-Z0-9]{8}$', parts[0]):
            ...
            with open(patient_file, 'w') as outfile:
                for line in infile:
                    if line.startswith('#') or line.startswith('AGE'):
                        ...
                        parts = line.strip().split('\t')
                        if len(parts) >= 1 and re.match(r'^P[A-Z0-9]{8}$', parts[0]):
